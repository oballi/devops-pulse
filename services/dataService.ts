...
export const fetchPostBySlug = async (slug: string): Promise<BlogPost | null> => {
    try {
        ...
        if (error || !data) {
            // Fallback: If not found by slug, try to find by ID if it looks like a UUID
            // This handles the case where we visit an old link /article/UUID
...
export const updatePost = async (id: string, post: Partial<BlogPost>): Promise<boolean> => {
    try {
        ...
        if (post.title) {
            ...
            // Opsiyonel: Başlık değişince slug da değişsin mi? 
            // SEO için URL değişimi risklidir ama yeni içerik için mantıklı olabilir.
            // Şimdilik sadece boşsa oluşturalım veya manuel tetikleyelim.
            // updates.slug = slugify(post.title); 
        }
